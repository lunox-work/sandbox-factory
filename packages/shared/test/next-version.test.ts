 ...
 * deploy with.
 *
 * It runs unattended on every merge to main and its answer becomes a permanent
 * public tag that the next release computes from, so the rule is pinned here.
 *
 * Imported by URL; see `build-info-resolver.test.ts`.
 */
...
// Not an error: ship.sh rejects these at the PR title, but a merge can arrive
// by other routes, and one unparseable commit must not abort a release.
