...
Release artifacts additionally carry a **signed provenance attestation**, which
is the part that is actually verifiable:
...
ongoing commitment and deliberately not attempted here.
...
The contract itself — the schema, `formatVersion`, `commitUrl`, `sameBuild` —
lives in `packages/shared/src/build-info.ts`, so all three surfaces format and
compare identically.
