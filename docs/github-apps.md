...
- Auto-review is on for non-draft PRs, skipping titles containing `chore(deps)`
  or `release`.
...
CodeRabbit is free on its Open Source plan for public repositories.
...
a label requires, so these will not appear on their own — create them by hand.
...
4. Create the `main protection` ruleset with its required checks, _after_ the
   first CI run so the check names exist to select from.
